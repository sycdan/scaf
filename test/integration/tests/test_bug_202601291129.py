...
def test_capability_only_added_when_necessary_for_deduplication():
    ...
    assert "scaf.create-action" in alias_names, f"Expected 'scaf.create-action' but got: {alias_names}"
    ...
    assert "scaf.user.create-action" not in alias_names, f"Should not have capability prefix: {alias_names}"
    assert "scaf.user.get-version" not in alias_names, f"Should not have capability prefix: {alias_names}"