...
- `scaf/cli.py` — Entry point; dynamically discovers and dispatches all user-facing commands
- `scaf/user/` — User-facing commands: `call/`, `config/`, `discover/`, `init/`, `serve/`, `version/`
...
All features must be built with TDD: write the test first (confirm it fails), then implement until it passes.
